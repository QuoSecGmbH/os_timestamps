...
struct profile_info_struct* profile_qt_delete_lastFile_empty(testenv_struct* env) {
    ...
    // QFile::remove(path_file_to_delete);
    QFile file(path_file_to_delete);
...
struct profile_info_struct* profile_qt_delete_notLastFile_empty(testenv_struct* env) {
    ...
    // QFile::remove(path_symlink);
    // QFile().remove(path_link);
    QFile file(path_link);
    bool isDelete = file.remove();
    Q_ASSERT(isDelete);
...
struct profile_info_struct* profile_qt_delete_directory(testenv_struct* env) {
    char* path_dir = misc_concat_ensure_dir_exists(env->dir_path, misc_concat(const_cast<char *>(__func__), "/"), 0, 0, env->output_file, env->error_file, __func__);
    char* path_directory_to_delete = misc_concat_ensure_dir_exists(path_dir, "directory_to_delete", s_0s, ns_after_open, env->output_file, env->error_file, __func__);
