#ifndef U_NEW_MKFIFO_C
#define U_NEW_MKFIFO_C


#include "u_new_mkfifo.h"

int check_utilities_new_mkfifo_new(FILE* csv_file, FILE* output_file, FILE* error_file, char* dir_path){
    char* path_dir = misc_concat_ensure_dir_exists(dir_path, "utilities.new.mkfifo.new/", 0, 0, output_file, error_file, __func__);
    ...
    char* path_fifo = misc_concat(path_targetdir, "fifo");
    
    char* command = "mkfifo ../targetdir/fifo; stat ../targetdir/fifo";
    
    char** watch_array = misc_char_array3(path_pwd, path_targetdir, path_fifo);
    int watch_num = 3;
...
int check_utilities_new_mkfifo_new_update(FILE* csv_file, FILE* output_file, FILE* error_file, char* dir_path){
    ...
    char* command = "mkfifo ../targetdir/fifo";
...
int check_utilities_new_mkfifo_new_mac(FILE* csv_file, FILE* output_file, FILE* error_file, char* dir_path){
    char* path_dir = misc_concat_ensure_dir_exists(dir_path, "utilities.new.mkfifo.new.mac/", 0, 0, output_file, error_file, __func__);
    ...
    char** watch_array = misc_char_array1(path_fifo);
    int watch_num = 1;
...
int check_utilities_new_mkfifo_existing(FILE* csv_file, FILE* output_file, FILE* error_file, char* dir_path){
    char* path_dir = misc_concat_ensure_dir_exists(dir_path, "utilities.new.mkfifo.existing/", 0, 0, output_file, error_file, __func__);
