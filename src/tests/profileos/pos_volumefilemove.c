#ifndef POS_VOLUMEFILEMOVE_C
#define POS_VOLUMEFILEMOVE_C
...
struct profile_info_struct* profileos_volumefilemove_interface_dir_new(testenv_struct* env){   
    ...
    char* path_srcdir = misc_concat_ensure_dir_exists(path_dir, "srcdir/", 0, 0, env->output_file, env->error_file, __func__);
    char* path_dstdir = misc_concat_ensure_dir_exists(path_dir_volume, "dstdir/", 0, 0, env->output_file, env->error_file, __func__);
    char* path = misc_concat_ensure_dir_exists_macb(path_srcdir, "src/", s_0s, ns_after_open, env->output_file, env->error_file, __func__);
