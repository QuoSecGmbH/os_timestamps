#ifndef I_FILE_R_GETDELIM_C
#define I_FILE_R_GETDELIM_C
...
int check_interfaces_file_r_getdelim(FILE* csv_file, FILE* output_file, FILE* error_file, char* dir_path){
    char* path = (char*) misc_concat_ensure_file_exists_filled(dir_path, "interfaces.file.r.getdelim", 10, s_0s, ns_100ms, output_file, error_file, __func__);
    ...
    char *line = NULL;
    size_t len = 0;
    int n_read = getdelim(&line, &len, 0x0a, fd);
...
int check_interfaces_file_r_getdelim_ungetc(FILE* csv_file, FILE* output_file, FILE* error_file, char* dir_path){
    char* path = (char*) misc_concat_ensure_file_exists_filled(dir_path, "interfaces.file.r.getdelim.ungetc", 10, s_0s, ns_100ms, output_file, error_file, __func__);
...
int check_interfaces_file_r_getdelim_ungetc_both(FILE* csv_file, FILE* output_file, FILE* error_file, char* dir_path){
    char* path = (char*) misc_concat_ensure_file_exists_filled(dir_path, "interfaces.file.r.getdelim.ungetc.both", 10, s_0s, ns_100ms, output_file, error_file, __func__);
    ...
    char *line1 = NULL;
    size_t len1 = 0;
    int n_read1 = getdelim(&line1, &len1, 0x0a, fd);
    
    char *line2 = NULL;
    size_t len2 = 0;
    int n_read2 = getdelim(&line2, &len2, 0x0a, fd);
